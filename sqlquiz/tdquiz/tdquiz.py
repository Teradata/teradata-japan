...
def setup_tdquiz(datadir="./data", verbose=True, param_file="_connect-info.json",
                 ...
                 database=None, encryptdata="true", dbs_port=1025, **params):
    params.update(dict(
        host=host, user=user, password=password or getpass("Password: "),
        database=database or user, encryptdata=encryptdata, dbs_port=dbs_port
    ))
    # Save the connection parameters to a json file,
    # so that the quiz notebooks can refer to it and avoid asking for password every time
    with open(param_file, "w") as f:
        json.dump(params, f)
        if verbose:
            print(f"Connection parameters are saved at '{param_file}'. Since it contains your password, use it with care.",
                  file=sys.stderr)
    ...
    pbar = tqdm(databases) if verbose else databases
    for dbdir in pbar:
        with Session(params) as session:
            database = Database(dbdir, session)
            if verbose:
                pbar.set_description(f"Create database '{database.dbname}'")
    ...
    pbar = tqdm(tables) if verbose else tables
    for tabledir in pbar:
        with Session(params) as session, Session(params) as session_insert:
            table = Table(tabledir, session, session_insert)
            if verbose:
                pbar.set_description(f"Create table '{table.table}'")
...
class TdquizMagic(Magics, Configurable):
    params = Dict(
        ...
        config=True,
        allow_none=True,
        help="Connecion parameters"
    )
    param_file = Unicode(
        "_connect-info.json",
        config=True,
        allow_none=True,
        help="JSON file that contains the connection parameters"
    )
    @line_cell_magic
    def tdquiz(self, line, cell=""):
        query = line or cell
        # load parameters from JSON file if params is invalid
...
def load_ipython_extension(ipython):
    ipython.register_magics(TdquizMagic)
